#!/usr/bin/env python3
"""Real-CASIA replication of the SIC-Gen mask-convention experiment on Christina's
enhanced-RL Level-1 feature (christina-fhe-fis/filter_fhe_iris_complete.py, imported
read-only), using real CASIA-Iris-Thousand codes extracted via Open Iris instead of
...
(see casia-extraction/manifest.json's "christina_pseudo_scales" note: her code treats
each of the 32 rows as an independent 1-row pseudo-scale, not "2 Gabor scales x 16
rows"). 300 identities (150 subjects x 2 eyes), each with a gallery (stem "1", image
00) and 3-9 probes (stems "2".."10"); identities/probe availability are read from
casia-extraction/manifest.json rather than assumed.
...
For each run: every probe is ranked against all 300 galleries by
...
_OUT_PATH = _RESULTS / "christina_realcasia_maskconvention.json"
...
def run_convention(identities: list, config: FilterFHEConfig, convention: str) -> dict:
    ...
    for ident in identities:
        ...
        if not ident["gallery_ok"]:
            continue  # none in this manifest, but guard for robustness
...
def main():
    ...
    n_probes_expected = sum(i["n_probes_ok"] for i in identities)
    print(f"Loaded manifest: {n_identities} identities, {n_probes_expected} expected probes")
    ...
    out = {
        "metadata": {
            "data_source": "casia-extraction/casia-codes-christina/ (real CASIA-Iris-Thousand via Open Iris)",
